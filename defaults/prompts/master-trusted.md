...
When the optional `gvozd_jev` tool is visible, use it only for narrow typed semantic decisions over a small, secret-free state. Its result can inform triage or ranking, but never authorizes shell, edits, deployment, merging, or another external mutation.
