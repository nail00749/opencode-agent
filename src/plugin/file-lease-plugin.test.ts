...
describe("file lease permission policy", () => {
  test("fails closed for readonly and unclaimed writer edits", () => {
    ...
    expect(enforceFileLeasePermission(readonly, configHolderOf(resolved), leases)).toBe(true)
    expect(readonly).toMatchObject({ effect: "deny", message: expect.stringContaining("readonly") })
    expect(enforceFileLeasePermission(writer, configHolderOf(resolved), leases)).toBe(true)
  ...
  test("unconfigured agents edit while idle and pause during writer leases", () => {
    ...
    expect(enforceFileLeasePermission(unknown, configHolderOf(resolved), leases)).toBe(false)
    expect(unknown.effect).toBe("allow")
    // A missing agent identity stays fail-closed even when idle.
    expect(enforceFileLeasePermission(missing, configHolderOf(resolved), leases)).toBe(true)
    ...
    expect(enforceFileLeasePermission(during, configHolderOf(resolved), leases)).toBe(true)
    ...
    expect(enforceFileLeasePermission(after, configHolderOf(resolved), leases)).toBe(false)
  })
  ...
  test("allows only every file in the current session lease", () => {
    ...
    expect(enforceFileLeasePermission(allowed, configHolderOf(resolved), leases)).toBe(false)
    expect(allowed.effect).toBe("allow")
    expect(enforceFileLeasePermission(denied, configHolderOf(resolved), leases)).toBe(true)
  ...
  test("fails closed for empty, unknown, and malformed edit resource mappings", () => {
    ...
    for (const event of [empty, unknown, incomplete, malformed]) {
      expect(enforceFileLeasePermission(event, configHolderOf(resolved), leases)).toBe(true)
      expect(event.effect).toBe("deny")
      expect(event.message).toBeTruthy()
    }
  })

  test("writer shell beyond the baseline escalates to a user ask, never-escalate stays denied", () => {
    const resolved = config()
    const leases = manager(resolved.projectRoot)
    const lease = leases.reserve({ parentSessionID: "master-1", agent: "back-fast", label: "backend", files: ["src/a.ts"] })
    leases.claim({ leaseId: lease.leaseId, sessionID: "child-1", parentSessionID: "master-1", agent: "back-fast" })
    const install = permission("back-fast", "shell", ["bun install"], "child-1", "ask")
    const destructive = permission("back-fast", "shell", ["git push --force origin main"], "child-1", "ask")
    const sudo = permission("back-fast", "shell", ["sudo rm -rf /"], "child-1", "ask")
    const coordinator = permission("master", "shell", ["bun test"], "master-1", "ask")

    // Regular out-of-baseline commands surface a user permission request.
    expect(enforceFileLeasePermission(install, configHolderOf(resolved), leases)).toBe(true)
    expect(install.effect).toBe("ask")
    expect(install.message).toContain("read-only verification")
    // Never-escalate command families stay hard denials.
    expect(enforceFileLeasePermission(destructive, configHolderOf(resolved), leases)).toBe(true)
    expect(destructive.effect).toBe("deny")
    expect(enforceFileLeasePermission(sudo, configHolderOf(resolved), leases)).toBe(true)
    expect(sudo.effect).toBe("deny")
    // The coordinator's shell also escalates to a user ask while leases are active.
    expect(enforceFileLeasePermission(coordinator, configHolderOf(resolved), leases)).toBe(true)
    expect(coordinator.effect).toBe("ask")
  })

  test("shellEscalation deny restores the hard block", () => {
    const resolved = { ...config(), lease: { ...config().lease, shellEscalation: "deny" as const } }
    const leases = manager(resolved.projectRoot)
    const lease = leases.reserve({ parentSessionID: "master-1", agent: "back-fast", label: "backend", files: ["src/a.ts"] })
    leases.claim({ leaseId: lease.leaseId, sessionID: "child-1", parentSessionID: "master-1", agent: "back-fast" })
    const install = permission("back-fast", "shell", ["bun install"], "child-1", "ask")
  ...
  test("writer runs read-only verification shell while other commands ask and mutations deny", () => {
    const resolved = config()
    const leases = manager(resolved.projectRoot)
    const lease = leases.reserve({ parentSessionID: "master-1", agent: "back-fast", label: "backend", files: ["src/a.ts"] })
    leases.claim({ leaseId: lease.leaseId, sessionID: "child-1", parentSessionID: "master-1", agent: "back-fast" })
    ...
    expect(enforceFileLeasePermission(verify, configHolderOf(resolved), leases)).toBe(false)
    // Mutating shell escalates to a user ask (the user may approve it).
    expect(enforceFileLeasePermission(mutate, configHolderOf(resolved), leases)).toBe(true)
    expect(mutate.effect).toBe("ask")
    expect(enforceFileLeasePermission(install, configHolderOf(resolved), leases)).toBe(true)
    expect(install.effect).toBe("ask")
    // The coordinator's shell escalates to a user ask while writer leases are active.
    expect(enforceFileLeasePermission(coordinator, configHolderOf(resolved), leases)).toBe(true)
    expect(coordinator.effect).toBe("ask")
    // Read-only roles keep the toolchain baseline while writer leases are active.
    expect(enforceFileLeasePermission(readonlyToolchain, configHolderOf(resolved), leases)).toBe(false)
    expect(enforceFileLeasePermission(unknown, configHolderOf(resolved), leases)).toBe(true)
    expect(unknown.effect).toBe("ask")
    expect(enforceFileLeasePermission(safeGit, configHolderOf(resolved), leases)).toBe(false)
  ...
  test("coordinator shell defers to agent rules when no leases are active", () => {
    ...
    // The moment a writer lease activates, the coordinator's shell escalates
    // to a user permission request.
    const lease = leases.reserve({ parentSessionID: "master-1", agent: "back-fast", label: "backend", files: ["src/a.ts"] })
    leases.claim({ leaseId: lease.leaseId, sessionID: "child-1", parentSessionID: "master-1", agent: "back-fast" })
    const during = permission("master", "shell", ["ssh -o BatchMode=yes serv hostname"], "master-1", "ask")
    expect(enforceFileLeasePermission(during, configHolderOf(resolved), leases)).toBe(true)
    expect(during.effect).toBe("ask")
...
describe("OpenCode file lease runtime", () => {
  ...
  test("passes an explicit case-sensitive filesystem override to the manager", async () => {
    const resolved = config()
    const runtime = await installFileLeaseRuntime(pluginHarness({}).context, configHolderOf(resolved), {
  ...
  test("rejects an invalid filesystem override before registering runtime resources", async () => {
    const resolved = config()
    const harness = pluginHarness({})
    await expect(installFileLeaseRuntime(harness.context, configHolderOf(resolved), {
...
describe("configured lease TTL", () => {
  test("manager receives the resolved config TTLs", () => {
    ...
    expect(resolved.lease.reservationTtlMs).toBe(540_000)
    expect(resolved.lease.activeTtlMs).toBe(2_700_000)
  })
})
...
describe("read-only agent shell during active writer leases", () => {
  test("toolchain commands are allowed while writer leases are active", () => {
    ...
    const event = { sessionID: "ses-git", agent: "git", action: "shell", resources: ["bun test *"], effect: "ask" as "allow" | "ask" | "deny" }
    // The plugin does not intervene: the host's pre-plugin effect ("ask"
    // here, simulating the agent's own ask-level rule) survives.
    expect(enforceFileLeasePermission(event, configHolderOf(config), manager)).toBe(false)
    expect(event.effect).toBe("ask")
  })

  test("non-baseline commands still escalate to a user ask during active writer leases", () => {
    const { config, manager } = runtimeFixture()
    const lease = manager.reserve({ parentSessionID: "ses-master", agent: "back-fast", label: "pkg", files: ["src/a.ts"] })
    manager.claim({ leaseId: lease.leaseId, sessionID: "ses-writer", parentSessionID: "ses-master", agent: "back-fast" })
    const event = { sessionID: "ses-git", agent: "git", action: "shell", resources: ["curl *"], effect: "ask" as "allow" | "ask" | "deny" }
    expect(enforceFileLeasePermission(event, configHolderOf(config), manager)).toBe(true)
    expect(event.effect).toBe("ask")
  })

  test("never-escalate commands stay denied during active writer leases", () => {
    const { config, manager } = runtimeFixture()
    const lease = manager.reserve({ parentSessionID: "ses-master", agent: "back-fast", label: "pkg", files: ["src/a.ts"] })
    manager.claim({ leaseId: lease.leaseId, sessionID: "ses-writer", parentSessionID: "ses-master", agent: "back-fast" })
    const event = {
      ...
      agent: "git",
      action: "shell",
      resources: ["GIT_OPTIONAL_LOCKS=0 git reset --hard HEAD~1"],
      effect: "ask" as "allow" | "ask" | "deny",
    }
    expect(enforceFileLeasePermission(event, configHolderOf(config), manager)).toBe(true)
    expect(event.effect).toBe("deny")
  })
